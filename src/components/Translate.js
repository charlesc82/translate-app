...
const useTranslation = (text, languages) => {
  ...
  useEffect(() => {
    ...
    doTranslation(text, languages, cancelToken, setTranslated);
  ...
  async (input, languageCode, cancelToken, callback) => {
    try {
      const { data } = await axios.post(
        "https://translation.googleapis.com/language/translate/v2?key=AIzaSyAU2ZXBnzNb9n_c_zycFhXVdwjDP02j_wo",
        {
          q: input,
          target: languageCode
        },
